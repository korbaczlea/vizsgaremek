...
function create_user(string $name, string $email, string $password_hash, ?string $phone = null): bool
{
    $pdo = get_db();
    $stmt = $pdo->prepare(
...
function update_phone(string $email, string $phone): bool
{
    $pdo = get_db();
    $stmt = $pdo->prepare(
        'UPDATE users
...
function is_admin_email(string $email): bool
{
    ...
    return strtolower(trim((string)($user['role'] ?? 'user'))) === 'admin';
}
